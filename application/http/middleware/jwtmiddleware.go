...
func (m *JWTMiddleware) Handler(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Println("an authentication error occurred. error: ", err)
			return
		}
...
func (m *JWTMiddleware) CheckJWT(w http.ResponseWriter, r *http.Request) error {
	...
	tokenDetails, err := ExtractTokenMetadata(r)
	if err != nil{
		m.logf("Error occurred while extracting token meta data")
		m.Options.ErrorHandler(w, r, "Error occurred while extracting token meta data")
		return err
	}

	tokenRemainingTimeValidity := getTokenRemainingValidity(tokenDetails.Exp)
	if tokenRemainingTimeValidity <= 0 {
		m.logf("Expired Token: Token has Expired")
		m.Options.ErrorHandler(w, r, "Token has Expired")
